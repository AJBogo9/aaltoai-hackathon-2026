...
analyze, so the walkthrough skills can read the output unchanged.

    python3 .pi/skills/analyze-fast/fast_report.py --workspace DIR
...
def marking(workspace):
    """The label this output inherits, and the providers cleared to read it.

    A report derived from a labelled folder is as confidential as the folder: the design
    system's rule is that the artifact is marked by the system it describes. The level is
    read from the workspace's own .confidentiality.json and the cleared providers from the
    policy the broker was started with, so neither is typed here. An unlabelled folder
    returns None, and the output then says nothing rather than guessing.
    """
    try:
        with open(os.path.join(workspace, ".confidentiality.json")) as fh:
            level = json.load(fh).get("level")
    except (OSError, ValueError):
        return None
    if not isinstance(level, str) or not level:
        return None

    policy_file = os.environ.get("PI_POLICY_FILE")
    if not policy_file or not os.path.exists(policy_file):
        # Outside the container, the policy is the repo's own.
        here = os.path.abspath(workspace)
        while True:
            candidate = os.path.join(here, ".pi", "confidentiality.json")
    ...
    try:
        with open(policy_file) as fh:
            policy = json.load(fh)
        levels, providers = policy["levels"], policy["providers"]
        order = list(providers)
        need = levels.index(level)
        cleared = sorted((p for p, c in providers.items() if levels.index(c) >= need),
                         key=lambda p: (levels.index(providers[p]), order.index(p)))
    except (OSError, ValueError, KeyError):
        cleared = []
    return {"level": level, "cleared": cleared}


def marking_line(mark):
    """The marking as one line of markdown, the same shape the README and the bands use."""
    return "`%s` · cleared: %s" % (mark["level"].upper(), ", ".join(mark["cleared"]) or "(none listed)")


def dump_report(path, rep, mark=None):
    ...
    `mark` is the workspace's label, written first so a reader sees what this
    file is before its contents. `audit` ignores fields it does not know.
    """
...
def main():
    ...
    mark = marking(workspace)
    lines = []
    if mark:
        lines += [marking_line(mark), ""]
    lines += ["# Fast pass - demo quality, low accuracy",
              "",
              f"Generated by analyze-fast from `{os.path.relpath(in_dir, workspace)}/` "
    ...
    for fname in files:
        ...
        dump_report(os.path.join(out_dir, fname + ".json"), rep, mark)
        f = rep["findings"]
        if not f:
