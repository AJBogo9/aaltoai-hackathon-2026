#!/usr/bin/env python3
"""Build a walkthrough session file from the existing audit reports in hyvätraportit/.

Reads hyvätraportit/summary.md + hyvätraportit/unit_*.csv.json, writes
hyvätraportit/walkthrough/session.json. Derives the four things the report schema
does not expose:
...
REPORTS = os.path.join(ROOT, "hyvätraportit")
...
def main():
    ...
    session = {
        "generated_from": "hyvätraportit/ (unmodified)",
