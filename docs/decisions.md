...
| Sun 20 Sep | Every marking is generated by `scripts/brand.py` from `tokens.json` plus `.pi/confidentiality.json`, and CI fails when one drifts | Six surfaces each held their own copy of the cleared provider list, and the GitHub banner had already gone stale: it still said `4 of 5` a day after `verda` was added |
| Sun 20 Sep | The terminal prints levels in 24 bit from the design system's own tokens, falling back to 4 bit, and `confidential` moved from yellow to blue | The deck's whole argument is that it quotes the machine. The machine was printing a colour that appeared nowhere else in the system |
